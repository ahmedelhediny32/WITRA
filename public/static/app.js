    ...
    if (Object.keys(usersByClient).length === 0) {
        ...
        clientsList.forEach(function(c) {
          ...
          if (cUsers && cUsers.length > 0) {
             ...
             html += '<summary style="padding:12px 16px; background:var(--paper); cursor:pointer; font-weight:600; display:flex; justify-content:space-between; align-items:center;">' + '<div style="display:flex; align-items:center; gap:8px;"><span class="accordion-arrow">▶</span> ' + esc(c.name) + '</div> <span class="status-badge" style="background:var(--canvas);color:var(--ink);">' + cUsers.length + ' ' + (cUsers.length === 1 ? esc(t("User")) : esc(t("Users"))) + '</span></summary>';
